...
from redis_video_stream.tasks import CeleryTaskManager, RedisStream, celery_app
...
async def reset_runtime_state():
    CeleryTaskManager.stop_all_stream.delay(DEFAULT_REDIS_URL)
    redis.Redis(host=DEFAULT_REDIS_HOST, port=DEFAULT_REDIS_PORT).flushdb()
